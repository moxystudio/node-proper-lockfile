...
const getMtimePrecision = require('./mtime-precision');
...
function acquireLock(file, options, callback) {
    ...
    options.fs.mkdir(getLockFile(file, options), (err) => {
        ...
        if (!err) {
            // If we've already determined mtime precision we're done
            if (mtimePrecision) {
                return options.fs.stat(getLockFile(file, options), callback);
            }

            return getMtimePrecision(getLockFile(file, options), options, (err, precision) => {
                if (err) {
                    return callback(err);
                }

                mtimePrecision = precision;
                options.fs.stat(getLockFile(file, options), callback);
            });
        }
...
function isEqualDates(d0, d1) {
    if (d0.getTime() === d1.getTime()) {
        return true;
    }

    if (mtimePrecision === 'ms') {
        return false;
    }
    ...
    throw new Error(`Unknown precision ${mtimePrecision}`);
}
...
function updateLock(file, options) {
    ...
    lock.updateTimeout = setTimeout(() => {
        ...
        options.fs.stat(getLockFile(file, options), (err, stat) => {
            ...
            const isMtimeOurs = isEqualDates(lock.mtime, stat.mtime);
...
function setMtimePrecision(value) {
    mtimePrecision = value;
}
...
module.exports.setMtimePrecision = setMtimePrecision;
