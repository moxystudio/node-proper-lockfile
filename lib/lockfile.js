    ...
    lock.updateTimeout = setTimeout(() => {
        ...
        options.fs.stat(getLockFile(file, options), (err, stat) => {
            ...
            options.fs.utimes(getLockFile(file, options), mtime, mtime, (err) => {
                ...
                // Ignore if the lock was compromised or released
                if (lock.compromised || lock.released) {
                    return;
                }
...
function setLockAsCompromised(file, lock, err) {
    // Signal the lock has been compromised
            ...
            acquireLock(file, options, (err, stat) => {
                ...
                callback(null, (releasedCallback) => {
                    if (lock.released) {
                        ...
                        if (!lock.compromised) {
                            error = Object.assign(new Error('Lock is already released'), { code: 'ERELEASED' });
                        }

                        return releasedCallback && releasedCallback(error);
                    }
