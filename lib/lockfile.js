    ...
    options.fs.mkdir(lockfilePath, (err) => {
        ...
        options.fs.stat(lockfilePath, (err, stat) => {
            ...
            removeLock(file, options, (err) => {
                ...
                acquireLock(file, { ...options, stale: 0 }, (err) => {
                    if (!err) {
                        // Only notify when the lock was actually acquired to make sure only one process can reclaim
                        options.onReclaimedStale && options.onReclaimedStale();
                    }
                    callback(err);
                });
            });
